...
- An independent versioned running programme: the unchanged 28-minute starter, ten beginner-to-continuous stages, 30–40 minute aerobic-base sessions, evidence-gated user-confirmed progression, and a later optional controlled hybrid session.
...
`npm run check` validates syntax, production structure, the strength and running programme catalogs, persistence requirements, versioned run timelines/assets, and automated tests.
...
See [Running domain](docs/RUNNING_DOMAIN.md), [run audio](docs/RUN_AUDIO.md), [Load guidance](docs/LOAD_GUIDANCE.md), [Progressive overload](docs/PROGRESSIVE_OVERLOAD.md), [Persistence](docs/PERSISTENCE.md), [Programme domain](docs/PROGRAMME_DOMAIN.md), [production audit](docs/PRODUCTION_AUDIT.md), [manual PWA checklist](docs/MANUAL_PWA_TEST_CHECKLIST.md), and [Product status](docs/PRODUCT_STATUS.md).
...
Voice, chimes-only, and visual-only guidance remain available for every versioned template. Voice/chimes use one continuous local media track and Media Session controls where supported; no browser speech or cloud text-to-speech is used. Lock-screen behavior varies by browser/OS, so use the in-app audio test. Maintainers can regenerate assets locally using [the run-audio guide](docs/RUN_AUDIO.md).
