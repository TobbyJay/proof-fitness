...
import { DATABASE_NAME, SCHEMA_V1, SCHEMA_V2, SCHEMA_V3 } from './schema.js';
import { migrateToVersion1, migrateToVersion2, migrateToVersion3 } from './migrations.js';
...
export class ProofFitnessDatabase extends Dexie {
  constructor(name = DATABASE_NAME, options = undefined) {
    ...
    this.on('populate',async transaction=>{await migrateToVersion1(transaction);await migrateToVersion2(transaction);await migrateToVersion3(transaction);});
  }
}
